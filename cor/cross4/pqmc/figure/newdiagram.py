...
files = sorted(glob.glob(r'rcorpair*.dat'))
for ff in files:
    file.append(ff)
    nf=nf+1
alldata=[]
...
for j in range(nf):
    plt.errorbar(x[j],y[j],yerr=dy[j],marker=next(marker),markerfacecolor=next(mface),
                 linestyle='',color=next(color1),label='$L=260$',capsize=2)
    ...
    for kk in range(100):
        if(x[j][kk] == x[j][10]):
            ...
            yf=AA*xf**BB
            break
    plt.plot(xf,yf,linestyle='-.',color=next(color11),label='$r^{-1}$')#,label='y=%5.3fx^%5.3f' % (AA,BB))
        #for kk in range(100):
        #    if(x[j][kk] == 68):
        #        #print(1)
        #        #print(min(x[j]),max(x[j]))
