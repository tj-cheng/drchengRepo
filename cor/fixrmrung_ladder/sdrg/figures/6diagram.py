...
files = sorted(glob.glob(r'rcorp*.dat'))
for ff in files:
    file.append(ff)
    nf=nf+1
alldata=[]
...
for j in range(nf):
    if(tcor[j][0]==4.0):
        ax1.errorbar(x[j],y[j],yerr=dy[j],marker=next(marker),markerfacecolor=next(mface),
                     linestyle='',color=next(color1),label='$P_{rm}=\\frac{%1.0f}{L}$' % (tcor[j][0]),capsize=2)
        ...
        for a, b, c in zip(x[j], y[j], tcor[j]):
            kk+=1
            ax1.text(a, b+0.8*(-1)**kk*b, '%.2f' % (1-c*1./a), ha='center', va='bottom', fontsize=6, color='blue')
    #if(tcor[j][0]==0.96):
    if(tcor[j][0]==6.0):
        ...
        for a, b, c in zip(x[j], y[j], tcor[j]):
            kk+=1
            ax2.text(a, b+0.8*(-1)**kk*b, '%.2f' % (1-c*1./a), ha='center', va='bottom', fontsize=6, color='blue')
    #if(tcor[j][0]==0.94):
    if(tcor[j][0]==8.0):
        ...
        for a, b, c in zip(x[j], y[j], tcor[j]):
            kk+=1
            ax3.text(a, b+0.8*(-1)**kk*b, '%.2f' % (1-c*1./a), ha='center', va='bottom', fontsize=6, color='blue')
    #if(tcor[j][0]==0.92):
    if(tcor[j][0]==20.0):
        for kk in range(100):
            #if(x[j][kk] == x[j][2]):
        ...
        for a, b, c in zip(x[j], y[j], tcor[j]):
            kk+=1
            ax4.text(a, b+0.8*(-1)**kk*b, '%.2f' % (1-c*1./a), ha='center', va='bottom', fontsize=6, color='blue')
    if(tcor[j][0]==30):
...
plt.savefig('pl_eecor.png')
plt.savefig('pl_eecor.pdf',bbox_inches="tight")
plt.show()
