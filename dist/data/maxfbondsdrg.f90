...
program probasic
   ...
   read(10,*)lx,ly0,rgprob
   read(10,*)init,samples,dd
   close(10)
   ...
   if(ly0 == 2) then 
      do i=1,samples
         ...
         do
            rgstep=rgstep+1
            call sdrgprocedure()
            call spinweight(rgstep,rank,rgprob)
         ...
         !   end if
            !--------------------------------!
            if (sum(bondsurv) .eq. 0) then 
               !print*,sum(lspn)
               exit
            end if
         end do
         !call writebindata(rank,rgprob)
         ! --- spins correlation --- !
...
!----------------------------!
subroutine spinweight(rgstep,rank,rgprob)
   use system; implicit none
   real(8) :: weight,rgprob,abspin
   integer :: rgstep,rank,s,nw,nws0,nws1
   ...
   weight=0.d0
   do s=1,nn
      if (spinsurv(s) .eq. 1) then
         nw=nw+1
         abspin=abs(lspn(s))
         !weight=weight+abs(lspn(s))
         weight=weight+abspin
         if (abspin .eq. 0.5) then
            nws0=nws0+1
         else if (abspin .eq. 1) then
            nws1=nws1+1
         end if 
      end if
   end do
   ...
   open(10,file='ladderevol'//trim(adjustl(numasstring)) //'.dat',status='unknown',position='append')
   write(10,'(i10,f18.10,i10,i10,i10,i10,f16.10,f16.10)') lx,wspin,nws0,nws1,nw,rgstep,rgprob,dd
   close(10)

end subroutine spinweight
!----------------------------!
...
subroutine sdrgprocedure()
   ...
   if (jj(nbmaxj) .lt. 0) fbond = fbond+1
...
!----------------------!
subroutine dijkstra(rank,rgprob)
   use system; implicit none
 ...
 if (sglabel(1,lx) .eq. 1) then
     open(10,file='2legladderlogcor'//trim(adjustl(numasstring)) //'.dat',status='unknown',position='append')
     write(10,'(i10, i10, f20.10, f20.10)') lx, ly0, 3./4., rgprob  !|C(L)|
     !write(10,*) lx, ly0, -1.d0*distance(ends)+log(3./4.), rgprob  !|C(L)|
     close(10)
 else 
     open(10,file='2legladderlogcor'//trim(adjustl(numasstring)) //'.dat',status='unknown',position='append')
     write(10,'(i10, i10, f20.10, f20.10)') lx, ly0, 0.d0, rgprob  !|C(L)|
     !write(10,*) lx, ly0, -1.d0*distance(ends)+log(3./4.), rgprob  !|C(L)|
     close(10)
 end if
...
subroutine writebindata(rank,rgprob)
   !-------------------------!
   use system; implicit none
   ...
   open(10,file='laddernumf'//trim(adjustl(numasstring)) //'.dat',status='unknown',position='append')
   write(10,'(i10,i10,f16.10,f16.10,i10)') lx,fbond,rgprob,dd,yon
   close(10)
   !print*,'rgprob',rgprob
   !if (ly0 .eq. 2) then
